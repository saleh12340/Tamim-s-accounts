...
class DatabaseHelper(private val ctx: Context) : SQLiteOpenHelper(ctx, DB_NAME, null, DB_VERSION) {
    ...
    override fun onOpen(db: SQLiteDatabase) {
        ...
        try {
            ...
            // Never make the activity crash just because an imported database has an older schema.
        }
    }
    ...
    private fun schema(db: SQLiteDatabase) {
        ...
        db.execSQL("CREATE TABLE IF NOT EXISTS reminders(id INTEGER PRIMARY KEY AUTOINCREMENT,customer_id INTEGER,reminder_date TEXT NOT NULL DEFAULT '',note TEXT DEFAULT '',is_completed INTEGER DEFAULT 0,FOREIGN KEY(customer_id) REFERENCES customers(id) ON DELETE CASCADE)")
        ...
        db.execSQL("CREATE TABLE IF NOT EXISTS suppliers(id INTEGER PRIMARY KEY AUTOINCREMENT,name TEXT NOT NULL DEFAULT '',phone TEXT DEFAULT '',balance REAL DEFAULT 0,notes TEXT DEFAULT '')")
        db.execSQL("CREATE TABLE IF NOT EXISTS expenses(id INTEGER PRIMARY KEY AUTOINCREMENT,title TEXT NOT NULL DEFAULT '',amount REAL NOT NULL DEFAULT 0,currency TEXT DEFAULT 'YER',date TEXT NOT NULL DEFAULT '',note TEXT DEFAULT '')")
        db.execSQL("CREATE TABLE IF NOT EXISTS invoices(id INTEGER PRIMARY KEY AUTOINCREMENT,type TEXT NOT NULL CHECK(type IN ('SALE','PURCHASE')),customer_id INTEGER,supplier_id INTEGER,date TEXT NOT NULL DEFAULT '',total REAL DEFAULT 0,paid REAL DEFAULT 0,note TEXT DEFAULT '',FOREIGN KEY(customer_id) REFERENCES customers(id) ON DELETE SET NULL,FOREIGN KEY(supplier_id) REFERENCES suppliers(id) ON DELETE SET NULL)")
        db.execSQL("CREATE TABLE IF NOT EXISTS invoice_items(id INTEGER PRIMARY KEY AUTOINCREMENT,invoice_id INTEGER NOT NULL,item_name TEXT NOT NULL DEFAULT '',quantity REAL NOT NULL DEFAULT 0,unit_price REAL NOT NULL DEFAULT 0,total REAL NOT NULL DEFAULT 0,FOREIGN KEY(invoice_id) REFERENCES invoices(id) ON DELETE CASCADE)")
        db.execSQL("CREATE TABLE IF NOT EXISTS settings(key TEXT PRIMARY KEY,value TEXT NOT NULL DEFAULT '')")
    ...
    private fun columns(db: SQLiteDatabase, table: String): Set<String> = try {
        db.rawQuery("PRAGMA table_info($table)", null).use { c ->
            val result = mutableSetOf<String>()
            while (c.moveToNext()) result += c.getString(1)
            result
        }
    } catch (_: Exception) { emptySet() }

    private fun addColumnIfMissing(db: SQLiteDatabase, table: String, column: String, definition: String) {
        if (!columns(db, table).contains(column)) {
    ...
    private fun ensureColumns(db: SQLiteDatabase) {
        addColumnIfMissing(db, "customers", "phone", "TEXT DEFAULT ''")
        addColumnIfMissing(db, "customers", "balance", "REAL DEFAULT 0")
        addColumnIfMissing(db, "customers", "group_id", "INTEGER")
        addColumnIfMissing(db, "customers", "notes", "TEXT DEFAULT ''")
        addColumnIfMissing(db, "transactions", "customer_id", "INTEGER")
        addColumnIfMissing(db, "transactions", "type", "TEXT DEFAULT 'DEBIT'")
        addColumnIfMissing(db, "transactions", "amount", "REAL DEFAULT 0")
        addColumnIfMissing(db, "transactions", "currency", "TEXT DEFAULT 'YER'")
        addColumnIfMissing(db, "transactions", "date", "TEXT DEFAULT ''")
        addColumnIfMissing(db, "transactions", "note", "TEXT DEFAULT ''")
        addColumnIfMissing(db, "transactions", "share_ref", "TEXT")
        addColumnIfMissing(db, "products", "barcode", "TEXT DEFAULT ''")
        addColumnIfMissing(db, "products", "buy_price", "REAL DEFAULT 0")
        addColumnIfMissing(db, "products", "sell_price", "REAL DEFAULT 0")
        addColumnIfMissing(db, "products", "stock", "REAL DEFAULT 0")
        addColumnIfMissing(db, "products", "min_stock", "REAL DEFAULT 0")
        addColumnIfMissing(db, "products", "notes", "TEXT DEFAULT ''")
        addColumnIfMissing(db, "expenses", "currency", "TEXT DEFAULT 'YER'")
        addColumnIfMissing(db, "expenses", "date", "TEXT DEFAULT ''")
        addColumnIfMissing(db, "expenses", "note", "TEXT DEFAULT ''")
    }
    ...
    override fun onUpgrade(db: SQLiteDatabase, oldVersion: Int, newVersion: Int) {
        ...
        ensureColumns(db)
        seed(db)
        try { recalculateAll(db) } catch (_: Exception) { }
    }

    private fun recalculateAll(db: SQLiteDatabase) {
    ...
    fun customers(): List<Customer> {
        return try {
            val a = mutableListOf<Customer>()
            readableDatabase.rawQuery("SELECT id,COALESCE(name,''),COALESCE(phone,''),COALESCE(balance,0),COALESCE(notes,'') FROM customers ORDER BY name COLLATE NOCASE", null).use { c ->
                while (c.moveToNext()) a.add(Customer(c.getLong(0), c.getString(1), c.getString(2), c.getDouble(3), c.getString(4)))
            }
            a
    ...
    fun addCustomer(name: String, phone: String = "", notes: String = ""): Long {
        ...
        return writableDatabase.insertOrThrow("customers", null, ContentValues().apply { put("name", name.trim()); put("phone", phone.trim()); put("notes", notes) })
    }

    fun updateCustomer(id: Long, name: String, phone: String, notes: String) {
        writableDatabase.update("customers", ContentValues().apply { put("name", name.trim()); put("phone", phone.trim()); put("notes", notes) }, "id=?", arrayOf(id.toString()))
    }

    fun deleteCustomer(id: Long) { writableDatabase.delete("customers", "id=?", arrayOf(id.toString())) }

    fun addTransaction(customerId: Long, type: String, amount: Double, note: String, currency: String = "YER", date: String = now()): Long {
        require(type == "DEBIT" || type == "CREDIT"); require(amount > 0)
        val id = writableDatabase.insertOrThrow("transactions", null, ContentValues().apply { put("customer_id", customerId); put("type", type); put("amount", amount); put("currency", currency); put("date", date); put("note", note) })
        recalculate(customerId); return id
    }

    fun updateTransaction(id: Long, type: String, amount: Double, note: String, currency: String) {
        val cid = transactionCustomer(id)
        writableDatabase.update("transactions", ContentValues().apply { put("type", type); put("amount", amount); put("note", note); put("currency", currency) }, "id=?", arrayOf(id.toString()))
        if (cid > 0) recalculate(cid)
    }
    ...
    fun transactions(customerId: Long): List<Tx> {
        val a = mutableListOf<Tx>()
        try { readableDatabase.rawQuery("SELECT id,type,amount,currency,date,COALESCE(note,'') FROM transactions WHERE customer_id=? ORDER BY date,id", arrayOf(customerId.toString())).use { c -> while (c.moveToNext()) a.add(Tx(c.getLong(0), c.getString(1), c.getDouble(2), c.getString(3), c.getString(4), c.getString(5))) } } catch (_: Exception) { }
        return a
    }
    ...
    fun products(): List<Product> {
        ...
        try { readableDatabase.rawQuery("SELECT id,name,barcode,buy_price,sell_price,stock,min_stock,notes FROM products ORDER BY name", null).use { c -> while (c.moveToNext()) a.add(Product(c.getLong(0), c.getString(1), c.getString(2), c.getDouble(3), c.getDouble(4), c.getDouble(5), c.getDouble(6), c.getString(7))) } } catch (_: Exception) { }
        return a
    }
    ...
    fun expenses(): List<Expense> {
        val a = mutableListOf<Expense>()
        try { readableDatabase.rawQuery("SELECT id,title,amount,currency,date,note FROM expenses ORDER BY date DESC,id DESC", null).use { c -> while (c.moveToNext()) a.add(Expense(c.getLong(0), c.getString(1), c.getDouble(2), c.getString(3), c.getString(4), c.getString(5))) } } catch (_: Exception) { }
        return a
    }

    fun deleteExpense(id: Long) { writableDatabase.delete("expenses", "id=?", arrayOf(id.toString())) }
    fun setting(key: String, default: String = "") = try { readableDatabase.rawQuery("SELECT value FROM settings WHERE key=?", arrayOf(key)).use { if (it.moveToFirst()) it.getString(0) else default } } catch (_: Exception) { default }
